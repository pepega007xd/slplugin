...
let from_astral_atom : SSL.t -> atom =
  let get_var = function SSL.Var var -> var | _ -> fail "unreachable" in
...
let substitute_by_fresh (var : var) (f : t) : t =
  substitute f ~var ~by:(mk_fresh_var_from var)
...
let make_var_explicit_src (var : var) (f : t) : t =
  ...
  | Some equiv_class ->
      let current_src =
        List.find (fun src -> List.exists (is_spatial_source src) f) equiv_class
      in
      swap_vars current_src var f
  | None -> f

let get_spatial_atom_from (src : var) (f : t) : atom option =
  f |> make_var_explicit_src src
  (* TODO: use better exception than Not_found *)
  |> List.find_opt (is_spatial_source src)
