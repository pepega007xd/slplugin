...
One of the approaches to static analysis is so-called _dataflow analysis_, which relies on traversing the control flow graph (CFG) of a program and tracking certain information at each node of the CFG. In this context, the information we are interested in is the values of pointers and the data structures on the program's heap. The _Frama-C_ framework described in @frama_c provides a generic framework for implementing a dataflow analysis of C programs.
...
Equality and inequality of variables behave as expected, but note that they require the heap to be empty. Therefore, the separating conjunction is used to join all atomic formulae instead of the regular conjunction. For example, a heap with one allocation will be described as $x |-> y * y = nil$. The formula $x |-> y and y = nil$ would be unsatisfiable, since the atom $y = nil$ describes an empty heap.

The $freed(x)$ atom says that the variable $x$ is not allocated. It has been added for the purpose of our analysis to explicitly mark freed variables. The key property is that adding $freed(x)$ to a formula containing $x$ discards its models where $x$ aliases with another allocated variable. For example, the formula $x |-> y$ has two models, one where $x$ and $y$ are distinct locations and another where they alias:
...
- $nls(x, y, z)$ describes a nested acyclic list. The top-level list leads through the `t` field from $x$ to $y$, but there is a sublist from each node's `n` field leading to $z$. The sublist itself is not a nested list, but a singly-linked list. Like the other lists, it can be empty (equivalent to $x = y$), but note that even the case of length one contains a sublist of an unbounded length.
...
#figure(
  caption: [Supported types of lists -- singly-linked list,\
...
One of the functions of Frama-C is to help with the development of custom analyses in the form of plugins. Frama-C handles command-line argument parsing, reports analysis results, and exposes APIs for access to the input CIL.
...
- `doStmt` -- This function is called before calling the transfer function; it receives the statement itself. The plugin has the option to stop the analysis of this statement or continue normally.
...
The analysis method is an extension of the approach described in @shape_analysis_1 and inspired by @shape_analysis_2. The analysis itself consists of multiple steps. First, the analyzed program is preprocessed into a form more suitable for analysis. Then the dataflow analysis over the modified AST is executed.
...
- `_const = var;` -- assertion that `var` is an allocated variable, `_const` is just a placeholder variable,
...
The analysis itself is implemented using the `Dataflow2` module provided by Frama-C (see @dataflow), and it is composed of these main parts. First, there is the transfer function implemented for the basic instructions described in @stmt_split. Then there are the simplifications applied to the formulae between instructions. The abstraction (over-approximation of pointer sequences by list predicates) is included in the simplifications. There is also the logic for the join operation. Finally, there is the specialization of formulae for each branch of a condition.

=== Transfer Function

The transfer function takes formulae describing the state before executing an instruction and changes them to reflect the state after the execution. The transfer function always processes the input formulae one by one, applying the operation described below to each input formula (here called $phi$) separately. When the materialization of a variable $x$ is done, the list predicate starting at $x$ is split into a points-to atom from $x$ and the rest of the list. If $x$ is not allocated in a formula, an invalid dereference is reported. The transfer function is implemented for each of the basic instructions:
...
- For an assignment with a field access on the right-hand side `a = b->f;`, the variable $b$ is first materialized in the formula, and then the target of $b$ at field $f$ is found (here called $t_b$). Then, a regular assignment is done with $t_b$ as the assigned variable. The resulting formula is $phi[f0\/a] * a = t_b$.

- When processing an assignment to a field `a->f = b;`, $a$ is first materialized, and then the points-to atom from $a$ is found and its field $f$ is changed to $b$. No substitution is done in this case because we are not overwriting the value of any variable.
...
The remaining instruction is a function call. If the function is from the allocation API (`malloc`, `calloc`, and `free`), it is handled differently from a user-defined function.
...
Between analyzing instructions, the state formulae are simplified and abstracted to ensure the termination of the analysis. A full description of some of these simplifications is provided in @simplifications. The following simplifications are applied in this order:
...
- Formulae different only in the length bound of a single spatial atom are merged, taking the lower bound of the two as the new bound. Points-to atoms are treated as lists with the length bound set to one for this simplification.
...
When the analysis reaches a statement for the second or subsequent times, a state #old is already associated with the statement. A new state is generated using the transfer function as usual, and then these states are joined to produce either a new state to be stored for the statement, or nothing if the original state covers the new state. By covering, we mean that all models of the new state are already included in the models of the original state. In other words, it must hold that $new entl old$, where #old and #new are disjunctions of individual SL formulae.

As an optimization, this entailment is not evaluated as-is, but is broken into a series of smaller entailments. The evaluation of satisfiability and entailment checks is cached, so this improves the efficiency of the caching because smaller entailments have a higher chance of appearing multiple times during analysis.
...
Note that after the analysis finds the fixpoint, the state is further simplified to:
...
#figure(
  ...
  move(
    ...
    grid(
      ...
      [], $x |-> f1 * y = x$,
      [], pass2($x |-> y * y |-> f2$),
      [], pass3($"ls"_(2+)(x, y) * y |-> f3$),
      c("  y->next = malloc(size);"), [],
      [], $x |-> f1 * f1 |-> f2 * y = x$,
      [], pass2($x |-> y * y |-> f2 * f2 |-> f3$),
      [], pass3($"ls"_(2+)(x, y) * y |-> f3 * f3 |-> f4$),
      c("  y = y->next;"), [],
      [], $x |-> y * y |-> f2$,
      [], pass2($x |-> f4 * f4 |-> y * y |-> f3$),
      [], pass3($"ls"_(2+)(x, f5) * f5 |-> y * y |-> f4$),
      c("}"), [],
      [], pass3($x |-> f1 * y = x$),
      [], pass3($"ls"_(1+)(x, y) * y |-> f2$),
      c("y->next = NULL;"), [],
      [], pass3($x |-> nil * y = x$),
      [], pass3($"ls"_(1+)(x, y) * y |-> nil$),
    ),
  ),
) <analysis_example>
...
This section describes various aspects of the analysis in more detail, with a focus on how certain parts of our method are implemented in the analyzer.
...
The conversion of all instructions in the program to only the basic instructions is done over multiple passes. First, all AST nodes irrelevant for the analysis are removed. This includes arithmetic and binary operations, type casts, integer comparison operations, and others. Then, variable initialization is replaced by simple assignments. Next, function call arguments with types irrelevant to the analysis are removed. _Relevant types_ are described in @type_analysis.
...
After this, all assignments into variables with irrelevant types are removed or replaced by a check that the dereferenced variable is allocated. At this point, all instructions in the program should be one of the basic instructions.

=== Preprocessing of Variables <vars_preprocessing>

Some of the preprocessing passes replace some expressions with special variables, namely `_nil` and `_const`. These are used to simplify the AST, instead of checking for multiple AST nodes such as a variable, a constant (zero), or a cast of a constant into a pointer type (`NULL`), all expressions use only variables. `_nil` is used to represent a `NULL` pointer, whereas `_const` replaces any expression that cannot be analyzed, such as arithmetic expressions or nondeterministic conditions. One other use of `_const` is that assignments into it are seen as an "allocation check" of the assigned variable. For example, `_const = var;` is not interpreted as a literal assignment into the variable `_const`, but as a check that `var` is allocated. These assignments are created during preprocessing, when a field access to a non-pointer variable is found. For example, the following statement:
...
This will not cause any change to the state formulae during dataflow analysis, but each formula will be checked to ensure that the variable `list` is allocated.
...
- Removing the `!` operator -- using the visitor `remove_not_operator` in #plugin_link("src/preprocessing/preprocessing.ml"), the following replacements are made:
  ...
  These replacements are done because the evaluation of many macros in the SV-COMP benchmarks creates exactly these types of conditions. Simply removing them is easier than implementing the analysis to cover them.
...
The purpose of this analysis is to determine which C types might implement one of the supported list variants, and to store this information for later use. Thanks to this, our analysis does not need any kind of annotation. This analysis pass is implemented in #plugin_link("src/preprocessing/types.ml").
...
The pass first iterates through all relevant types in the code and applies the following heuristic to determine if the type is one of the supported linked lists. The result of this heuristic is one of:
- singly-linked list,
- doubly-linked list,
- nested list,
- generic struct.

First, all relevant fields of a structure are recursively analyzed to get their heuristic result. Then,
...
The type used to represent formulae in Astral has a tree structure that requires pattern matching to access every term. This makes sense for a solver that needs to support formulae of all shapes on its input, but for the purpose of this analysis, we need a flattened representation of the exact shape of the formulae we use. This will avoid having to pattern match terms for which we already know the type, and it will allow us to define shorthands for predicates that have to be expressed by a more complex structure in Astral's type. The formula type, along with a series of functions for manipulating these formulae, is implemented in #plugin_link("src/formula.ml").
...
Variables used by the formulae are represented using the original Astral type. Program variables have the same name as their corresponding C variables, while logic variables are distinguished by names containing the character `!`. When adding a logic variable to a formula, we need to give it a fresh name that does not appear anywhere in the formula.
...
The module also provides functions for manipulating spatial atoms, such as getting and changing the target of a points-to atom, checking that a variable is allocated in the formula, or converting points-to atoms to lists. To represent the type of field of a points-to atom, the type `field_type` is used. It is defined in #plugin_link("src/preprocessing/types.ml") in the following way:

```ocaml
type field_type = Next | Prev | Top | Other of string | Data
```

The `Other` variant is used to represent arbitrary fields using their name.
...
- The other option is for the list to have a length of zero. In this case, it is replaced with an equality, and `materialize` is called recursively.
...
The first formula represents the case where the original list $ls(bound: 0, x, y)$ had a length of at least one. The second formula represents the case of the list being empty. Note that the materialization also handles explicitly making $x$ the source of the points-to atom.
...
`Eq` represents a list of variables that form an equivalence class. Equivalences in formulae are not handled arbitrarily as pairs of equivalent variables, but a minimal list of disjoint equivalence classes is maintained throughout the analysis. When adding a new equivalence into a formula, the `add_eq lhs rhs` function is used. This function first checks if any of the newly equivalent variables are present in any of the existing equivalent classes, and then:
...
- If $x != y$ is added to $dls(bound: 0+\/1, x, y, p, n)$, it becomes $dls(bound: 2, x, y, p, n)$.
...
The reachable subformula is a merge of reachable spatial atoms and filtered equivalence classes, inequalities, and #freed atoms. The unreachable subformula is then composed of all atoms of the original formula not present in the reachable subformula.
...
After creating the new context, the initial state for the first statement in the called function is set to the modified input formula, and the analysis is started on this statement. After the analysis finishes, the result state is read out from the result hashmap, and the original context is restored.
...
Simplifications are done on the formulae of the state between the analysis of two instructions to reduce the size and the number of variables in the formulae to speed up solver queries. Most of the simplification passes are implemented in #plugin_link("src/simplification.ml") and called from the `doEdge` function inside #plugin_link("src/analysis.ml").
...
All logic variables are then substituted with this substitution target. After this, all equivalence classes are deduplicated, getting rid of the variables duplicated during substitution. Finally, equivalence classes with fewer than two variables are again removed.
...
The sorting is done by defining a comparison function that accepts two formulae and puts them in the correct order. This function first calculates the lowest number of allocations for each of the formulae and then puts the higher of the two on the left side of the entailment. This makes sense because the formula with the higher number of allocations can have a subset of the formula with the lower number of allocations, but not the other way around. For example, this entailment holds:
...
`-sl-edge-abstraction` enables the abstraction of formulae on every edge between two statements, not just at the end of a loop. This can significantly decrease the analysis times when enabled because the abstraction is done sooner, but can lead to less precise results on some programs because of earlier over-approximation.
...
The KTSN analyzer was evaluated on a small set of crafted programs and on a larger set of SV-COMP benchmarks @svcomp to compare it to existing tools.
...
The evaluated version of the analyzer is marked with the Git tag `final_code` in the tool's repository at #link("https://github.com/pepega007xd/ktsn", raw("https://github.com/pepega007xd/ktsn")).
...
The benchmarked programs all have an expected result. The expected result can either be a violation of one of the properties, or `true`, signifying that all properties are valid for the program.
...
Since many analyzers do not support data structures of unbounded size, the benchmarks contain a number of programs that work with lists of small, bounded size. To improve the precision of our analysis on these benchmarks, we unroll a fixed number of iterations on loops.
...
Compared to PredatorHP, which reaches 119 of its 124 correct results in under a second, the analysis is overall slower. However, CPAchecker does not produce any results in less than 7 seconds, so compared to this tool, our analysis times are overall better.
...
#figure(
  caption: [Examples of the fastest and slowest benchmarks\
    with time spent in the solver versus the analyzer itself],
  table(
    columns: 5,
...
#figure(
  caption: [
    The times of evaluating queries when analyzing `sll-shared-sll-after.c`\
    (logarithmic time axis)
  ],
  image("queries.svg"),
)

There is usually a small number of queries that take up most of the time spent in the solver. For example, during the analysis of `sll-shared-sll-after.c`, the top 8 slowest queries take 18.2 seconds of the total 19.9 seconds spent in the solver.
...
This thesis introduced a static analyzer focused on verifying the memory safety of C programs. The tool uses dataflow analysis with separation logic formulae as the value domain. The analyzer implements abstraction using inductive predicates, which allows it to analyze list structures of unbounded size.

The analyzer was tested on a set of crafted benchmark programs that show it can prove the correctness of programs that create unbounded lists of all supported types. These benchmarks also verify that the analyzer is able to correctly detect use-after-free errors, null-pointer dereferences, double-free errors, and memory leaks.

Further benchmarking was done on the `LinkedLists` subset of the SV-COMP benchmark collection. Of the 134 total programs, our analyzer can find 80 correct results. From the 18 analyzers that competed in SV-COMP 2025, only two others achieve better results than ours.
...
The main drawback of our method, preventing us from analyzing more programs, is the lack of support for pointer arithmetic. Another disadvantage compared to other analyzers is the lack of analysis of numeric values, which makes the analysis of most conditions impossible. Because of this, most programs containing bugs are reported as unknown results instead of detected errors, because the analyzer cannot be sure that the detected bug is real, and not just a false-positive caused by an imprecise analysis of a condition.
